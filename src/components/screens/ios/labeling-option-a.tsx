...
export function LabelingOptionA({ onNavigate, initialFeedback, initialShowContestEnded, earnState, warmupRemaining, onWarmupProgress, onThresholdComplete }: LabelingOptionAProps) {
  const [feedback, setFeedback] = useState<FeedbackState>(initialFeedback ?? "none");
  const [sessionEarnings, setSessionEarnings] = useState(0);
  const [readCount, setReadCount] = useState(0);
  const [activeCount, setActiveCount] = useState(0);
  const [showContestEnded] = useState(initialShowContestEnded ?? false);
  ...
  return (
    <div className="h-full relative overflow-hidden" style={{ background: "#0a0a0a", fontFamily: "var(--ios-font)" }}>
      ...
      {/* ── Contest ended sheet ── */}
      {showContestEnded && (
        <div className="absolute inset-0 flex flex-col px-6 pt-16 pb-10" style={{ background: "var(--ios-surface-default)", fontFamily: "var(--ios-font)" }}>
          <div
